...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete sharkbyte database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no sharkbyte datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old sharkbyte datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected sharkbyte 0.7 datadir."
    ;;
  3)
    echo "Detected sharkbyte pre-0.8 datadir."
    ;;
  4)
    echo "Detected sharkbyte 0.8 datadir."
    ;;
esac
