...
// which are part of the sharkbyte repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="sharkbyte-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /sharkbyte-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "sharkbytepay-sharkbyte/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from sharkbytepay-sharkbyte/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'sharkbytepay-sharkbyte/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from sharkbytepay-sharkbyte/develop"
            }
          }

          if (hasCache) {
            sh "cd /sharkbyte-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /sharkbyte-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /sharkbyte-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /sharkbyte-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /sharkbyte-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /sharkbyte-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /sharkbyte-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
