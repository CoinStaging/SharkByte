...
const char * const BITCOIN_CONF_FILENAME = "sharkbyte.conf";
const char * const BITCOIN_PID_FILENAME = "sharkbyted.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "sharkbyte" is a composite category enabling all sharkbyte-related debug output
                if(ptrCategory->count(std::string("sharkbyte"))) {
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/sharkbyte";
#else
    // Unix
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty sharkbyte.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override sharkbyte.conf
