...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all sharkbyte related functionality
...
/*
    ...
    allow endless transaction that would bloat sharkbyte and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all sharkbyte specific functionality
