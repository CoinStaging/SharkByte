...
# sharkbyte_hash
RUN git clone https://github.com/sharkbytepay/sharkbyte_hash
RUN cd sharkbyte_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} sharkbyte
RUN useradd -u ${USER_ID} -g sharkbyte -s /bin/bash -m -d /sharkbyte sharkbyte
...
RUN mkdir /sharkbyte-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /sharkbyte-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /sharkbyte-src
